...
def bb_algorithm(_n):
    ...
    pq = Priority_Queue()
    pq.push(_n, lambda _n : _n.pessimistic)
    # podados = 0
    # explorados = 1
    ...
    while not pq.isEmpty():
        ...
        # se obtiene el mejor elemento de la cola y se elimina
        node = pq.top()
        
        if is_leaf(node):  

            if node.current_weight <= best_v:
                best_v = node.current_weight
                best_sol = node.sol
        ...
        for i in range(1, len(Node.m)):
            ...
            if not is_feasible(aux_path[i]) :
                ...
                if n.pessimistic < best_v:
                    best_v = n.pessimistic

                if n.optimistic > best_v or n.current_weight > best_v or n.optimistic > n.pessimistic:
                    # podados = podados + 1
                    continue 
...
#     path[i] = True
#     n = Node(1, i, Node.m[1,i], path, [])
#     path[i] = False

#     x = threading.Thread(target=bb_algorithm, args=(n,))
#     threads.append(x)
#     x.start()
